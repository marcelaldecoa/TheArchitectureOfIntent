...
│   ├── evolve/                        ← Part 5: closed loop, adoption playbook, governance, cost, telemetry, anti-patterns, framework versioning, DevSquad mapping + evolve/scenarios/
...
│   ├── examples/                      ← three legacy worked pilots (customer support, code-gen pipeline, coding agent); superseded by phase scenarios in Parts 1–5
│   └── appendices/                    ← glossary, pattern index, reading paths, companion paper, references, archetype card, RACI card, MCP & skills card, model-tier card
