...
- **Focus-and-structure pass — Part 5 split, IDS as Foundations exit, principles-first Adoption Playbook, pattern justification audit (v2.4.0):** addresses five reviewer findings in one pass. (1) **Part 5 → Parts 5 + 6**: Part 5 returns to *EVOLVE* (closed loop, anti-patterns, framework versioning, MVP-AoI, deployment patterns) and a new **Part 6 — OPERATIONS** holds the seven sustaining-ops chapters (governance, cost & latency, cacheable prompts, telemetry, adoption playbook, DevSquad mapping & co-adoption), which move from `src/evolve/` to a new `src/operate/` dir. Part 6 is explicitly *not* a sixth activity — the activity count stays five; deck/paper sync check remains green. Part 6 (Reference) renumbers to Part 7. (2) **IDS anchored as Foundations exit**: `foundations/07-intent-design-session.md` gains a "Why this is the Foundations exit" section; each Part's opening chapter gains a *Where this sits in the work* pointer back to it. (3) **Adoption Playbook leads with principles**: new "Principles before the playbook" section in `operate/05` names four principles (tight-focus, demonstration-beats-mandate, rigid-on-load-bearing, survive-the-champion); the procedural walkthrough is reframed as "A concrete rhythm" — one worked example, not THE rhythm. (4) **`foundations/08` clarified**: senior-engineer chapter gets a "Who this chapter is for" header naming it as audience-specific and optional on first read. (5) **Pattern Justification Map**: `appendices/pattern-index.md` gains a new section auditing all ~50 patterns against the canonical 12-section spec template; all map cleanly, density concentrates in §11/§8/§6. MINOR because no commitment moves.
