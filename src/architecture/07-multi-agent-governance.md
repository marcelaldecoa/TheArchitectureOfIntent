...
- [Coding Agents](../agents/08-coding-agents.md) — when coding-agent deployment is multi-agent (Devin-style), the governance treatment lives here
- [Evals and Benchmarks](../operating/07-evals-and-benchmarks.md) — multi-agent evals require system-level acceptance plus per-seam contract checks
- [Red-Team Protocol](../operating/08-red-team-protocol.md) — multi-agent systems have specific adversarial surfaces (cross-agent injection, handoff manipulation)
