...
**Framework Version**
A semantic-ish version number for the Architecture of Intent as a whole. The version applies to the framework's load-bearing commitments — the five archetypes, the four dimensions, the seven failure categories, the four oversight models, the four signal metrics, the **five** activities, composition as a first-class design surface — as instantiated by the book and the companion paper *together*. **MAJOR** bumps mark structural changes that break existing specs (a sixth archetype, a removed Cat, promoting a sub-activity to a peer activity); **MINOR** bumps mark additions that don't (a new chapter, a new pattern, a new spec sub-block); **PATCH** bumps mark prose, link, and figure refinements. Current version: **v2.0.0** (2026-05-10) — the stable v2.0 release. The MAJOR bump promoted *Evolve* to a peer fifth activity, introduced three running scenarios end-to-end across the five activities (customer-support agent, coding-agent pipeline, internal docs Q&A built by a DevSquad team), reorganized the book around the activity spine, redrew the framework canvas to a five-row layout, added the field-guide voice (vignette-before-exposition openings) across the conceptual chapters, and aligned chapter subtitles to the v2.0.0 Part placement. The companion paper acquired §5 (one canonical scenario walked end-to-end) as part of the same release. Shipped through eight release candidates (rc1–rc8) before stabilizing. See [`CHANGELOG.md`](https://github.com/marcelaldecoa/TheArchitectureOfIntent/blob/main/CHANGELOG.md) at the repository root.
