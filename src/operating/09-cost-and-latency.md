...
Cost and latency engineering produces direct inputs to the **cost-per-correct-output** metric ([Four Signal Metrics](06-metrics.md)):
...
- [The Canonical Spec Template](../sdd/07-canonical-spec-template.md) — the **Cost Posture** sub-block in §4 is the upstream surface this chapter sits above; Section 7 is where non-functional cost and latency constraints live
...
- [Four Signal Metrics](06-metrics.md) — cost-per-correct-output is the metric this work moves
- [Evals and Benchmarks](07-evals-and-benchmarks.md) — eval suite must include cost and latency regressions, not just behavioral regressions
...
- [Coding Agents](../agents/08-coding-agents.md) — coding agents have characteristic cost profiles (long context, multi-tool loops) that this chapter's principles apply to directly
- [Multi-Agent Governance](../architecture/07-multi-agent-governance.md) — multi-agent systems are where cost and latency penalties compound most quickly
