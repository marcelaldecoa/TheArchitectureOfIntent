...
A serious protocol fixes all three: it tests beyond the spec's positive distribution, runs continuously, and connects every finding to the [Spec Gap Log](../sdd/06-living-specs.md) and the [Constraint Library](../repertoires/templates/constraint-library.md).
...
1. **Spec Gap Log** ([The Living Spec](../sdd/06-living-specs.md)) — every successful exploit is a gap in the spec, manifest, or oversight model.
...
For [Computer-Use Agents](../agents/09-computer-use-agents.md), the attack surface includes shapes that don't exist for text-only agents. These belong in a dedicated stratum of the pre-launch and monthly batteries:
