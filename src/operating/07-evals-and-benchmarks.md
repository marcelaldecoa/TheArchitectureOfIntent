...
Every failing eval — at any of the four levels — is a candidate Spec Gap Log entry. The connection is mechanical:

- **Level 1 failure** → tool contract or runtime issue, not usually a spec gap
- **Level 2 failure** → the spec said the agent should do X; the agent did not do X. This is either a Category 1 (Spec Failure: the spec was self-contradictory) or Category 6 (Model-Level Failure: the model is not capable enough). Diagnose with the protocol from [Failure Modes and How to Diagnose Them](../theory/05-failure-as-design-signal.md).
- **Level 3 regression** → the agent's behavior changed in a way the spec did not anticipate. Often Category 1 (the spec was incomplete and the previous behavior worked by accident) or Category 6 (model upgrade introduced new failure modes — see [Model Upgrade Validation](../patterns/deployment/model-upgrade.md)).
- **Level 4 distribution shift** → the spec was correct for the original distribution and is now incomplete for the current one. Category 1, with a "spec needs to grow" annotation.

This is what closes the loop between offline eval and the [Living Spec](../sdd/06-living-specs.md): every regression and every drift event becomes a spec evolution candidate, captured in the gap log.
...
- [The Canonical Spec Template](../sdd/07-canonical-spec-template.md) — Section 9 acceptance criteria are the input to the spec acceptance suite
...
- [The Living Spec](../sdd/06-living-specs.md) — the gap log that closes the loop from eval failure to spec evolution
